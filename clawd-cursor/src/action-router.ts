...
const APP_ALIASES: Record<
  string,
  { processNames: string[]; searchTerm: string; macOSAppName?: string }
> = {
  ...
  notepad: {
    ...
    searchTerm: "Notepad",
    macOSAppName: "TextEdit",
  },
  calculator: {
    processNames: ["Calculator", "calc"],
    searchTerm: "Calculator",
    macOSAppName: "Calculator",
  },
  calc: {
    processNames: ["Calculator", "calc"],
    searchTerm: "Calculator",
  ...
  cmd: {
    processNames: ["cmd"],
    searchTerm: "Command Prompt",
    macOSAppName: "Terminal",
  },
  terminal: {
    processNames: ["WindowsTerminal", "cmd", "Terminal"],
    searchTerm: "Terminal",
    macOSAppName: "Terminal",
  },
  powershell: {
  ...
  word: {
    ...
    searchTerm: "Word",
    macOSAppName: "Microsoft Word",
  },
  excel: {
    processNames: ["EXCEL"],
    searchTerm: "Excel",
    macOSAppName: "Microsoft Excel",
  },
  vscode: {
    processNames: ["Code"],
    searchTerm: "Visual Studio Code",
    macOSAppName: "Visual Studio Code",
  },
  code: {
    processNames: ["Code"],
    searchTerm: "Visual Studio Code",
  ...
  settings: {
    ...
    searchTerm: "Settings",
    macOSAppName: "System Settings",
  },
  "system settings": {
    processNames: ["System Preferences", "System Settings"],
    searchTerm: "System Settings",
    macOSAppName: "System Settings",
  },
  "task manager": {
    processNames: ["Taskmgr"],
    searchTerm: "Task Manager",
    macOSAppName: "Activity Monitor",
  },
  "activity monitor": {
    processNames: ["Activity Monitor"],
    searchTerm: "Activity Monitor",
    macOSAppName: "Activity Monitor",
  },
  figma: {
    processNames: ["Figma"],
  ...
  teams: {
    ...
    searchTerm: "Teams",
    macOSAppName: "Microsoft Teams",
  },
  discord: {
    processNames: ["Discord"],
    searchTerm: "Discord",
    macOSAppName: "Discord",
  },
  codex: {
    processNames: ["Codex"],
    searchTerm: "Codex",
  ...
  wezterm: {
    processNames: ["WezTerm", "wezterm"],
    searchTerm: "WezTerm",
    macOSAppName: "WezTerm",
  },
  iterm: {
...
export class ActionRouter {
  ...
  async route(subtask: string): Promise<RouteResult> {
    ...
    const separatorVerb =
      /(?:,\s*|\band\s+then\s+|\bthen\s+)\b(open|launch|start|type|click|tap|save|close|find|press|navigate|go to|draw|write|send|create|delete|move|drag|search|download|upload)\b/i;
    const verbAndVerb =
      /^(open|launch|start|type|click|tap|navigate|go to|find|press)\b.+\band\s+(open|launch|start|type|click|tap|save|close|find|press|navigate|go to|draw|write|send|create|delete|move|drag|search|download|upload)\b/i;
    if (separatorVerb.test(task) || verbAndVerb.test(task)) {
      this.telemetry.llmFallbacks += 1;
      return {
        handled: false,
        description: "Compound task — needs decomposition",
      };
    }
    ...
    const typeMatch = rawTask.match(
      /^(?:type|enter|input)\s+(?:['"]([^'"]*)['"]\s*|(\S.*))$/i,
    );
    if (typeMatch) {
      this.telemetry.nonShortcutHandled += 1;
    ...
    const urlMatch = rawTask.match(
      /^(?:go to|navigate to|visit|browse to|open)\s+(https?:\/\/[^\s]+|www\.[^\s]+|[^\s.]+\.\w{2,}(?:\/[^\s]*)?)$/i,
    );
    if (urlMatch) {
      this.telemetry.nonShortcutHandled += 1;
    ...
    const shortcutMatch = findShortcut(task, PLATFORM, {
      contextHint: shortcutContext,
      enableFuzzy: true,
    });
    if (shortcutMatch) {
      this.telemetry.shortcutHits += 1;
      if (shortcutMatch.matchType === "fuzzy")
        this.telemetry.shortcutFuzzyHits += 1;
    ...
    const focusMatch = rawTask.match(
      /^(?:focus|switch to|bring up|activate|go to)\s+(\S.*)$/i,
    );
    if (focusMatch) {
      this.telemetry.nonShortcutHandled += 1;
    ...
    if (winCtrlMatch) {
      this.telemetry.nonShortcutHandled += 1;
      return this.handleWindowControl(
        winCtrlMatch[1].toLowerCase(),
        winCtrlMatch[2].trim(),
      );
    }

    // 10. "find <text>" → Ctrl/Cmd+F then type
    // Only match in-page find patterns, NOT general "search for X" which should go to LLM
    const mod = PLATFORM === "darwin" ? "Super" : "Control"; // Cmd on macOS, Ctrl on Windows/Linux
    const findMatch = rawTask.match(
      /^(?:find|search in page|search within|search on page|find on page|find in page)\s+(.+)$/i,
    );
    if (findMatch) {
  ...
  private async handleOpenApp(appName: string): Promise<RouteResult> {
    ...
    try {
      ...
      if (running) {
        ...
        const result = await this.a11y.focusWindow(
          undefined,
          running.processId,
        );
        if (result.success) {
  ...
  private findWindowForApp(
    ...
    if (alias) {
      for (const pn of alias.processNames) {
        ...
        const minimized = windows.find(
          (w) => w.processName.toLowerCase() === pn.toLowerCase(),
        );
    ...
    return windows.find((w) => w.title.toLowerCase().includes(normalizedName));
  }

  private async launchViaStartMenu(
    searchTerm: string,
    macOSAppName?: string,
  ): Promise<RouteResult> {
  ...
  private async waitForAppReady(
      ...
      try {
        ...
        for (const w of currentWindows) {
          ...
          const matchesProcess = alias
            ? alias.processNames.some(
                (pn) => w.processName.toLowerCase() === pn.toLowerCase(),
              )
            : false;
          ...
          if (matchesProcess || matchesTitle || matchesSearch) {
            console.log(
              `   ✅ Window detected: "${w.title}" (pid:${w.processId}) — settling ${READY_SETTLE_MS}ms`,
            );
    ...
    console.log(
      `   ⚠️ Readiness timeout (${READY_TIMEOUT}ms) — app may still be loading`,
    );
    return null;
  }
  ...
  private async handleNavigateToUrl(url: string): Promise<RouteResult> {
    ...
    try {
      ...
      if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        return {
          handled: false,
          description: `Invalid URL protocol: ${parsed.protocol}`,
      ...
      return {
        handled: false,
        description: `Invalid URL: ${fullUrl}`,
        error: "Invalid URL",
      };
    }
    ...
    try {
      ...
      const browser = windows.find((w) =>
        BROWSER_PROCESSES.some((bp) =>
          w.processName.toLowerCase().includes(bp),
        ),
      );
  ...
  private async handleClick(elementName: string): Promise<RouteResult> {
    try {
      ...
      return {
        handled: false,
        description: `Element "${elementName}" not found via a11y`,
      };
    } catch (err) {
      return {
        handled: false,
        description: `Click failed: ${err}`,
        error: String(err),
      };
    }
  }
  ...
  private async handleFocusWindow(windowName: string): Promise<RouteResult> {
    ...
    try {
      ...
      return {
        handled: false,
        description: `Window "${windowName}" not found`,
      };
    } catch (err) {
      return {
        handled: false,
        description: `Focus failed: ${err}`,
        error: String(err),
      };
    }
  }
  ...
  private async handleClose(appName: string): Promise<RouteResult> {
    try {
      ...
      return {
        handled: false,
        description: `Cannot close — window "${appName}" not found`,
      };
    } catch (err) {
      return {
        handled: false,
        description: `Close failed: ${err}`,
        error: String(err),
      };
    }
  }
  ...
  private async handleWindowControl(
    action: string,
    appName: string,
  ): Promise<RouteResult> {
    try {
      ...
      if (!focusResult.handled) {
        return {
          handled: false,
          description: `Cannot ${action} — window "${appName}" not found`,
        };
      }
  ...
  private async handleKeyPress(keyDesc: string): Promise<RouteResult> {
    ...
    try {
      ...
      return {
        handled: false,
        description: `Key press failed: ${err}`,
        error: String(err),
      };
    }
  }
