...
import { buildGeminiConfig, testGeminiConnection } from "./gemini-client";
import {
  PROVIDERS,
  detectProvider,
  buildPipeline,
...
export async function quickSetup(): Promise<PipelineConfig | null> {
  ...
  if (!anyAvailable) {
    console.log(
      "⚠️  No AI providers detected. Layer 1 (Action Router) will still work.",
    );
    return null;
  }
  ...
  if (workingText.length === 0 && workingVision.length === 0) {
    console.log(
      "⚠️  No working models found. Layer 1 (Action Router) will still work.",
    );
    return null;
  }
 ...
 */
async function quickTestAllProviders(
  ...
  for (const scan of scanResults) {
    ...
    // ── Gemini/Vertex AI: use @google/genai SDK instead of HTTP ──
    if (scan.key === "gemini" || scan.key === "vertexai") {
      promises.push(testGeminiModelAsync(scan.key, provider.textModel, "text"));
      promises.push(
        testGeminiModelAsync(scan.key, provider.visionModel, "vision"),
      );
      continue;
    }
...
export async function runDoctor(opts: {
  ...
  if (
    opts.apiKey &&
    (opts.provider || opts.baseUrl || opts.textModel || opts.visionModel)
  ) {
  ...
  const unavailableCloud = scanResults.filter(
    (s) => !s.available && s.key !== "ollama",
  );
  if (unavailableCloud.length > 0) {
    ...
    const keyInfo: Record<string, string> = {
      anthropic:
        "ANTHROPIC_API_KEY — https://console.anthropic.com (vision + computer use)",
      openai: "OPENAI_API_KEY — https://platform.openai.com (GPT-4o vision)",
      gemini:
        "GEMINI_API_KEY — https://aistudio.google.com (Gemini vision + computer use)",
    ...
    console.log(
      `      Set in .env file or as environment variable, then re-run: clawdcursor doctor`,
    );
    ...
    if (process.stdin.isTTY && process.stdout.isTTY) {
      ...
      const keyInput = await new Promise<string>((resolve) =>
        rlSetup.question(
          "\n   🔑 Paste an API key now to add a provider (or Enter to skip): ",
          resolve,
        ),
      );
      rlSetup.close();
      ...
      if (trimmedKey) {
        ...
        if (detectedKey && PROVIDERS[detectedKey]) {
          ...
          if (matchingScan) {
            ...
            console.log(
              `   ✅ Detected ${PROVIDERS[detectedKey].name} key! Testing...`,
            );
            ...
            try {
              ...
              console.log(
                `   ⚠️ Could not save to .env — set ${envVarName} manually`,
              );
            }
          }
        } else {
          console.log(
            `   ⚠️ Could not detect provider for this key. Set it manually in .env`,
          );
        }
      }
    }
  ...
  for (const test of modelTests) {
    ...
    const latency = test.latencyMs
      ? `${test.latencyMs}ms`
      : test.error || "failed";
    console.log(
      `   ${test.role === "text" ? "Text:  " : "Vision:"} ${test.model} (${providerName}) ${icon} ${latency}`,
    );
  }
  ...
  if (workingText.length > 0) {
    results.push({
      ...
      ok: true,
      detail: workingText
        .map((t) => `${t.model} via ${t.providerKey}`)
        .join(", "),
    });
  } else {
    results.push({
      name: "Text model",
      ok: false,
      detail: "No working text model found",
    });
  }
  ...
  if (workingVision.length > 0) {
    ...
    results.push({
      name: "Vision model",
      ok: false,
      detail: "No working vision model found",
    });
  }
  ...
  console.log(
    `   Layer 2: ${pipeline.layer2.enabled ? `${pipeline.layer2.model} via ${providerNameForUrl(pipeline.layer2.baseUrl)}` : "DISABLED"} ${pipeline.layer2.enabled ? "✅" : "❌"}`,
  );
  console.log(
    `   Layer 3: ${pipeline.layer3.enabled ? `${pipeline.layer3.model} via ${providerNameForUrl(pipeline.layer3.baseUrl)}` : "DISABLED"} ${pipeline.layer3.enabled ? "✅" : "❌"}`,
  );
...
async function runSingleProviderFlow(
  ...
  if (textResult.ok) {
    ...
    results.push({
      name: `Text model (${textModel})`,
      ok: false,
      detail: textResult.error || "Failed",
    });
    ...
    if (providerKey !== "ollama") {
      ...
      try {
        ...
        if (ollamaRes.ok) {
          ...
          const ollamaModels = (ollamaData.models || []).map(
            (m: { name: string }) => m.name,
          );
          const bestModel = pickOllamaTextModel(ollamaModels);
          if (bestModel) {
            const ollamaResult = await testModel(
              PROVIDERS["ollama"],
              "",
              bestModel,
              false,
            ...
            if (ollamaResult.ok) {
              ...
              textModel = bestModel;
              console.log(
                `   ✅ Ollama ${bestModel}: ${ollamaResult.latencyMs}ms (fallback)`,
              );
            } else {
  ...
  if (apiKey) {
    ...
    if (visionResult.ok) {
      ...
      results.push({
        name: `Vision model (${visionModel})`,
        ok: false,
        detail: visionResult.error || "Failed",
      });
  ...
  if (
    providerKey !== "ollama" &&
    pipeline.layer2.model &&
    !pipeline.layer2.baseUrl
  ) {
    ...
    try {
      ...
      /* not Ollama model, leave baseUrl as-is */
    }
  }
  ...
  console.log(
    `   Layer 2: Accessibility Reasoner → ${pipeline.layer2.enabled ? pipeline.layer2.model : "DISABLED"} ${pipeline.layer2.enabled ? "✅" : "❌"}`,
  );
  console.log(
    `   Layer 3: Screenshot → ${pipeline.layer3.enabled ? pipeline.layer3.model : "DISABLED"} ${pipeline.layer3.enabled ? "✅" : "❌"}`,
  );
  if (pipeline.layer3.computerUse) {
    console.log(`   🖥️  Computer Use API: enabled (Anthropic native)`);
  }
...
function buildPipelineFromSelection(
  ...
  const primaryProviderKey =
    selected.layer3?.providerKey || selected.layer2?.providerKey || "ollama";
  ...
  const layer2Provider = selected.layer2
    ? PROVIDERS[selected.layer2.providerKey] || PROVIDERS["ollama"]
    : primaryProvider;
  const layer3Provider = selected.layer3
    ? PROVIDERS[selected.layer3.providerKey] || PROVIDERS["ollama"]
    : primaryProvider;
    ...
    try {
      ...
      const gpus = await Promise.all(
        entries.map(async (d: Record<string, unknown>) => {
          const name =
            (d["sppci_model"] as string | undefined) ||
            (d["_name"] as string | undefined) ||
            "Unknown GPU";
          // Discrete GPUs (Intel/AMD/NVIDIA on older Macs) expose VRAM directly.
          const vram =
            (d["spdisplays_vram"] as string | undefined) ||
            (d["spdisplays_vram_shared"] as string | undefined);
          ...
          if (gpuCores) {
            let unifiedMem = "";
            try {
              const { stdout: memOut } = await execFileAsync("sysctl", [
                "-n",
                "hw.memsize",
              ]);
...
async function promptPipelineSelection(
  ...
  const recommendedText = recommended.layer2.enabled
    ? {
        providerKey:
          providerKeyForUrl(recommended.layer2.baseUrl) ||
          recommended.providerKey,
        model: recommended.layer2.model,
      }
    : null;
  const recommendedVision = recommended.layer3.enabled
    ? {
        providerKey:
          providerKeyForUrl(recommended.layer3.baseUrl) ||
          recommended.providerKey,
        model: recommended.layer3.model,
      }
    : null;
...
async function promptCategoryChoice(
  ...
  options.forEach((opt, idx) => {
    ...
    const recommendedMark =
      recommendedChoice &&
      opt.providerKey === recommendedChoice.providerKey &&
      opt.model === recommendedChoice.model
        ? " ★ recommended"
        : "";
    const latency = opt.latencyMs ? `, ${opt.latencyMs}ms` : "";
    console.log(
      `   ${idx + 1}. ${opt.model} (${providerName}${latency})${recommendedMark}`,
    );
  });
  ...
  if (
    ...
    console.log(
      `   Invalid choice "${trimmed}". Using default ${defaultIndex + 1}.`,
    );
    const selected = options[defaultIndex];
    return { providerKey: selected.providerKey, model: selected.model };
  }
...
function isLikelyVisionOnly(modelId: string): boolean {
  ...
  return (
    ...
    lower.startsWith("bakllava") ||
    lower.startsWith("moondream")
  );
}

/**
 * Test a Gemini/Vertex AI model using the @google/genai SDK.
 */
async function testGeminiModelAsync(
  providerKey: string,
  model: string,
  role: "text" | "vision",
): Promise<ModelTestResult> {
  const config = buildGeminiConfig({ model });
...
function savePipelineConfig(
  ...
  const layer2ProviderKey =
    providerKeyForUrl(pipeline.layer2.baseUrl) || pipeline.providerKey;
  const layer3ProviderKey =
    providerKeyForUrl(pipeline.layer3.baseUrl) || pipeline.providerKey;
...
function printNoProvidersHelp(results: DiagResult[]): void {
  ...
  console.log(
    `      Then: ollama pull <model>  (e.g. qwen2.5:7b, llama3.2, gemma2)\n`,
  );
  console.log(`   Option 2 (Cloud):`);
  console.log(`      Get an API key from any OpenAI-compatible provider:`);
  console.log(
    `      - Anthropic: https://console.anthropic.com (has Computer Use)`,
  );
...
function printSummary(results: DiagResult[], pipeline: PipelineConfig): void {
  ...
  if (allOk) {
    ...
    if (textFailed) {
      console.log(
        `   Text LLM missing — needed for accessibility reasoning (Layer 2)`,
      );
      console.log(
        `   Free (local):  ollama pull <model> && ollama serve  (e.g. qwen2.5:7b, llama3.2)`,
      );
      console.log(
        `   Cloud:         clawdcursor install --provider <provider> --api-key YOUR_KEY`,
      );
      console.log("");
    }
    if (visionFailed) {
      console.log(
        `   Vision LLM missing — needed for screenshot analysis (Layer 3)`,
      );
      console.log(
        `   Run:           clawdcursor install --provider <provider> --api-key YOUR_KEY`,
      );
      console.log(
        `   Supported:     Any provider with vision models (Anthropic, OpenAI, Groq, etc.)`,
      );
      console.log("");
    }
    if (visionFailed && !textFailed) {
      console.log(
        `   ℹ️  Running without vision — action router + accessibility reasoner handle most tasks.`,
      );
    }
  }
  console.log("");
...
async function registerOpenClawSkill(results: DiagResult[]): Promise<void> {
  ...
  try {
    ...
    if (!skillsDir) {
      ...
      console.log(
        "   💡 Install OpenClaw (https://openclaw.ai) to use Clawd Cursor as an AI skill",
      );
      return;
    }
    ...
    try {
      ...
      results.push({
        name: "OpenClaw skill",
        ok: true,
        detail: "Registered (symlink created)",
      });
    } catch (symlinkErr) {
      ...
      try {
        ...
        results.push({
          name: "OpenClaw skill",
          ok: true,
          detail: "Registered (SKILL.md copied)",
        });
      } catch (copyErr) {
  ...
  try {
    ...
    const res = await fetch(
      "https://api.github.com/repos/AmrDab/clawd-cursor/releases/latest",
      {
        headers: {
          ...
          "User-Agent": "clawd-cursor-doctor",
        },
    ...
    if (res.ok) {
      ...
      if (
        ...
        console.log(
          `   ⬆️  Update available: v${latestTag} (you have v${currentVersion})`,
        );
        const updateCmd =
        ...
        results.push({
          name: "Version",
          ok: true,
          detail: `v${currentVersion} (latest)`,
        });
      }
    } else {
      // GitHub API rate limit or error — skip gracefully
      console.log(
        `   ✅ v${currentVersion} (update check skipped — GitHub API returned ${res.status})`,
      );
      results.push({
        name: "Version",
        ok: true,
    ...
    try {
      ...
      results.push({
        name: "Version",
        ok: true,
        detail: `v${pkg.version} (update check unavailable)`,
      });
    } catch {
      results.push({
        name: "Version",
        ok: true,
        detail: "unknown (update check unavailable)",
      });
    }
  }
}
      ...
      if (data.type === "error" && data.error) {
        ...
        const msg =
          typeof err === "object" && err !== null
            ...
            : String(err);
        const hint =
          err.type === "not_found_error" || err.type === "invalid_request_error"
    ...
    return {
      ...
      layer3: {
        ...
        apiKey: layer3ComputerUse
          ? explicitLayer3ApiKey ||
            resolveProviderApiKey(layer3ProviderKey, defaultApiKey)
          : undefined,
      },
    };
  } catch {
